...
SYSTEM_PROMPT = """You are J.A.R.V.I.S., Tony Stark's AI. British, witty, concise. Address user as "sir." Keep responses 1-3 sentences. Dry wit. When asked to create an app, output JSON: {"message":"response","requires_code":true/false,"project_type":"react","specification":{"components":[],"features":[],"styling":"","backend_requirements":[]}}"""
...
async def voice_command(file: UploadFile = File(...)):
    try:
        ...
        # For cloud without Whisper, return text-input mode message
        return JSONResponse({
            "transcript": "Cloud mode active",
            "response": {
                "message": "Voice transcription unavailable on cloud instance, sir. Use the text chat endpoint at /api/chat for conversation.",
                "requires_code": False
            },
            "audio_response": None
        })
